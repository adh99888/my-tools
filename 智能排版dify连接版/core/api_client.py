...
class APIClient:
    ...
    def call_ai_api(self, content: str, model_id: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 准备请求数据
            api_url, headers, data = self._prepare_request(
                prompt, content, model_config, api_key
            )
            ...
            response = self.session.post(
                api_url, headers=headers, json=data, timeout=self.timeout
            )
            response.raise_for_status()

            # 解析响应
            result = self._parse_response(
                response.json(), model_config.get("provider", "deepseek")
            )
...
12. 特别重要：请返回完整文档，不要压缩、删减或总结内容，保持与原文相近的长度
...
请直接返回优化后的完整文档内容。"""
    ...
    def _prepare_request(
        self, prompt: str, content: str, model_config: Dict[str, Any], api_key: str
        ...
        # 智能计算max_tokens：移除硬编码的32768限制，根据模型能力和内容长度动态设置
        max_tokens_config = model_config.get("max_tokens", 8192)

        # 估算输入token数（中文字符约1:2，英文字符约1:1.3）
        char_count = len(content)
        # 粗略估算：中文字符为主，按1:2.5计算
        estimated_input_tokens = int(char_count * 2.5)

        # 计算合理的输出token数
        # 规则1：输出token数不超过模型最大token数的70%（留30%给输入）
        # 规则2：输出token数至少为输入token数的80%（保证不压缩）
        max_output_by_model = int(max_tokens_config * 0.7)
        min_output_for_content = int(estimated_input_tokens * 0.8)

        # 选择较大的值，确保不压缩内容
        desired_max_tokens = max(min_output_for_content, max_output_by_model)

        # 对于不同提供商，设置上限
        if provider == "dashscope":
            # 阿里云可能有自己的限制，但先使用计算值
            final_max_tokens = min(desired_max_tokens, 32768)  # 保持兼容性
        elif provider == "moonshot":  # Kimi
            # Kimi支持大上下文，可以使用更大值
            final_max_tokens = min(desired_max_tokens, 65536)
        else:
            # 其他模型使用计算值，但不超过配置值
            final_max_tokens = min(desired_max_tokens, max_tokens_config)

        logger.debug(
            f"max_tokens计算: 配置={max_tokens_config}, 输入≈{estimated_input_tokens}tokens, 输出设置={final_max_tokens}tokens"
        )
